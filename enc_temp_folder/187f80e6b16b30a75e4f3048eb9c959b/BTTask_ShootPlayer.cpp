			...
			if (APlayerCharacter* Player = Cast<APlayerCharacter>(UGameplayStatics::GetPlayerPawn(GetWorld(), 0)))
			{
				...
				if (hitDetected)
				{
					if (Hit.GetActor())
					{
						...
						UGameplayStatics::ApplyDamage(Hit.GetActor(), Enemy->ShotDamage, AIController, Enemy, UDamageType::StaticClass());
						UE_LOG(LogTemp, Warning, TEXT("Hit Actor Shot: %s"), *Hit.GetActor()->GetName());
					}
				}
